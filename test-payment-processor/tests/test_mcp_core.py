"""
Core MCP server tests for TestPaymentProcessor
...
async def test_mcp_server_initialization(mcp_server):
    ...
    assert mcp_server.server.name == "test-payment-processor-mcp-server"
...
async def test_capabilities_tool(mcp_server):
    ...
    assert module_info["name"] == "test-payment-processor"
    assert module_info["type"] == "CORE"
    assert module_info["domain"] == "payments"
...
async def test_api_schema_resource(mcp_server):
    ...
    assert schema["info"]["title"] == f"TestPaymentProcessor MCP Server API"
