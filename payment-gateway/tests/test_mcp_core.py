"""
Core MCP server tests for PaymentGateway
...
async def test_mcp_server_initialization(mcp_server):
    ...
    assert mcp_server.server.name == "payment-gateway-mcp-server"
...
async def test_capabilities_tool(mcp_server):
    ...
    assert module_info["name"] == "payment-gateway"
...
async def test_api_schema_resource(mcp_server):
    ...
    assert schema["info"]["title"] == f"PaymentGateway MCP Server API"
