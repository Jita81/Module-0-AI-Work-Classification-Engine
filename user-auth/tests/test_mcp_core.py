"""
Core MCP server tests for UserAuth
...
async def test_mcp_server_initialization(mcp_server):
    ...
    assert mcp_server.server.name == "user-auth-mcp-server"
...
async def test_capabilities_tool(mcp_server):
    ...
    assert module_info["name"] == "user-auth"
    assert module_info["type"] == "CORE"
    assert module_info["domain"] == "authentication"
...
async def test_api_schema_resource(mcp_server):
    ...
    assert schema["info"]["title"] == f"UserAuth MCP Server API"
