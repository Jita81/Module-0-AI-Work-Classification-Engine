"""
Core MCP server tests for ClaudeApiGateway
...
from ..core import ClaudeApiGatewayMCPServer
from ..types import ClaudeApiGatewayConfig, ClaudeApiGatewayResult
...
async def mcp_server():
    ...
    server = ClaudeApiGatewayMCPServer(config)
...
async def test_mcp_server_initialization(mcp_server):
    ...
    assert mcp_server.server.name == "claude-api-gateway-mcp-server"
...
async def test_capabilities_tool(mcp_server):
    ...
    assert module_info["name"] == "claude-api-gateway"
    assert module_info["type"] == "INTEGRATION"
    assert module_info["domain"] == "ai-services"
...
async def test_api_schema_resource(mcp_server):
    ...
    assert schema["info"]["title"] == f"ClaudeApiGateway MCP Server API"
