# Integration Guide: ClaudeApiGateway MCP Server

## Overview

This guide explains how to integrate with the ClaudeApiGateway MCP Server for ai-services domain operations.
...
async def integrate_with_claude_api_gateway():
...
kind: Service
metadata:
  name: claude-api-gateway-mcp-service
  annotations:
    mcp.protocol/version: "2024-11-05"
    mcp.discovery/enabled: "true"
spec:
  selector:
    app: claude-api-gateway-mcp-server
  ports:
  - port: 8000
    targetPort: 8000
```
...
def register_mcp_server():
    ...
    consul_client.agent.service.register(
        name="claude-api-gateway-mcp-server",
        service_id="claude-api-gateway-instance-1",
        port=8000,
        tags=["mcp", "ai-discoverable", "ai-services"],
        meta={
...
LOG_LEVEL=DEBUG python3 claude-api-gateway_server.py
```
